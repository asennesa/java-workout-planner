...
public interface UserRepository extends SoftDeleteRepository<User, Long> {
    ...
    /**
     * Find active user by Auth0 user ID.
     * Used for Auth0 authentication to sync user profiles with local database.
     * 
     * @param auth0UserId the Auth0 user ID (e.g., "auth0|507f1f77bcf86cd799439011")
     * @return Optional containing the user if found and not deleted
     */
    ...
    /**
     * Check if Auth0 user ID exists.
     * 
     * @param auth0UserId the Auth0 user ID to check
     * @return true if Auth0 user ID exists and is not deleted, false otherwise
     */
    @Query("SELECT CASE WHEN COUNT(u) > 0 THEN true ELSE false END FROM User u WHERE u.auth0UserId = :auth0UserId AND u.deleted = false")
